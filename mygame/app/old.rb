...
class Game
  ...
  def render_player
    flip_horizontally = state.player.facing == -1
  ...
  def calc_input
    ...
    if state.player.action == :standing
      state.player.dx += 0.1 * inputs.left_right
      state.player.dx = state.player.max_speed * state.player.dx.sign if state.player.dx.abs > state.player.max_speed
    end
    ...
    can_jump = state.player.jump_at.elapsed_time > 10 if state.player.jump_count <= 1

    return unless was_jump_requested && can_jump

    state.player.action = :standing if state.player.action == :slash_6
    state.player.dy = 1
  ...
  def calc
    ...
    return unless state.player.y <= 0 && state.player.dy < 0
  ...
  def calc_player_movement
    ...
    state.player.dx = 0 if state.player.dx.abs < 0.09
  ...
  def calc_requested_action
    return unless state.player.requested_action
  ...
  def calc_sabre
    ...
    sabre_throws = %i[throw_0 throw_1 throw_2]
    unless sabre_throws.include? state.player.action
  ...
  def calc_next_action
    return unless state.player.next_action_queue[Kernel.tick_count]
    ...
    if state.player.action == :slash_0
      ...
      state.player.dy  = 1.0 if is_air_born
      ...
      state.player.dy = 0 if state.player.dy < 0
      state.player.dy += if is_air_born
                           1.0
                         else
                           1.0
                         end
      ...
      state.player.dy = 0 if state.player.dy > 0
      state.player.dy  = -0.5 if is_air_born
